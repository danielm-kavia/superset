...
interface ResetExploreConfigAction {
  type: typeof actions.RESET_EXPLORE_CONFIG;
}
...
export default function exploreReducer(
  ...
  const actionHandlers: ActionHandlers = {
    ...
    [actions.RESET_EXPLORE_CONFIG]() {
      const { initialFormData, initialDatasource } = state;

      // Old Explore sessions without a hydration snapshot remain unaffected.
      if (!initialFormData || !initialDatasource) {
        return state;
      }

      // A reset with no configuration changes is intentionally safe and does
      // not create a redundant state update.
      if (
        isEqual(state.form_data, initialFormData) &&
        isEqual(state.datasource, initialDatasource) &&
      ...
      const resetState: ExploreState = {
        ...state,
        datasource: cloneDeep(initialDatasource),
        form_data: cloneDeep(initialFormData),
        hiddenFormData: undefined,
        controlsTransferred: [],
      };
