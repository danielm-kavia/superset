...
interface ResetExploreConfigurationAction {
  type: typeof actions.RESET_EXPLORE_CONFIGURATION;
}
...
type ExploreAction =
  ...
  | SetFormDataAction
  | ResetExploreConfigurationAction
...
export default function exploreReducer(
  ...
  const actionHandlers: ActionHandlers = {
    ...
    [actions.RESET_EXPLORE_CONFIGURATION]() {
      if (!state.initialFormData) {
        return state;
      }

      const formData = { ...state.initialFormData };
      return {
        ...state,
        // A partial Explore state may not yet have datasource metadata. Form
        // data can still be safely reset, while control state must wait until
        // a datasource is available for datasource-specific initialization.
        controls: state.datasource
          ? (getControlsState(
              state as Parameters<typeof getControlsState>[0],
              formData,
            ) as ControlStateMapping)
