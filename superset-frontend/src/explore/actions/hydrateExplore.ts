...
export const hydrateExplore =
  ...
  (dispatch: Dispatch, getState: () => ExplorePageState) => {
    ...
    // Keep a stable, normalized configuration snapshot for Explore's reset
    // action without sharing nested values with subsequently edited controls.
    const initialFormDataSnapshot = cloneDeep(initialFormData);
